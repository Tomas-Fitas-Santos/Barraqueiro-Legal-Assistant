...
function TutorialAnalysisWorkspace({ kind, step, state, fixture, documentUrl, onTarget }: { kind: TutorialKind; step: TutorialStep; state: Record<string, unknown>; fixture: Fixture; documentUrl: string; onTarget: () => void }) {
  ...
  const tab: 'detalhes' | 'chat' | 'historico' = historyIds.has(step.id) ? 'historico' : step.id === 'open-work' ? 'detalhes' : 'chat';
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
    ...
    <AnalysisTabs tab={tab} tutorialTargets={{ chat: step.id === 'open-work' ? 'open-findings' : undefined, historico: step.id === 'open-history' ? 'history-tab' : undefined }} onSelect={(selected) => { if ((step.id === 'open-work' && selected === 'chat') || (step.id === 'open-history' && selected === 'historico')) onTarget(); }} />
